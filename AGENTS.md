...
| Benchmarks          | `pnpm bench` runs the microbenchmark suite locally (tinybench wall-clock). CI runs the same suite under CodSpeed instrumentation on every push to `main`/`develop` — deterministic, hardware-agnostic. Treat any regression on the dashboard as a real issue, not noise. See "Performance & Benchmarks" below.                                                                                  |
...
| Transitive deps     | Root `package.json` may define `pnpm.overrides` for patched transitive versions when upstream packages lag; remove or narrow overrides when installs no longer need them. Published `@cutoff/audio-ui-*` packages ship `dist/` only—Vite and Vitest stay dev-only in the monorepo                                                                                                               |
...
| Testing             | Vitest 3; most `.test.tsx` colocated in `src/`; render-count regression fixtures and their `.test.tsx` live under `packages/react/test/render-count/` so they never land in `dist/`; mock deps; React 18/19 compat                                                                                                                                                                              |
| Build               | Library: Vite 6 with TS declarations (`vite-plugin-dts` 4); demo: Next.js 15 with Turbopack; run `pnpm build && pnpm typecheck`                                                                                                                                                                                                                                                                 |
...
| Library styles      | Library CSS must never affect the client application. Use only selectors scoped to `.audioui` or `.audioui-*` (e.g. `.audioui svg text`). No unqualified element selectors (e.g. `input`, `textarea`, `svg`, `text`) in library stylesheets. All exported components that render library UI apply `CLASSNAMES.root` (`.audioui`) on their root element. See `agents/audioui-styling-system.md`. |
...
| CSS entry points    | Default: `style.css` (includes default font Oxanium). Opt-out: `style-no-font.css` for apps that supply their own font; set `--audioui-font-family` and `--audioui-font-size` if needed.                                                                                                                                                                                                        |
...
- **Cursor Behavior**:
  ...
  - DiscreteControl, BooleanControl: Pointer cursor when interactive (any paired callback or `onClick`), no cursor otherwise. Keys: pointer when `onNoteChange` or `onClick` is provided.
...
- **Library styles**: Library CSS must never affect the client application. All selectors in library stylesheets are scoped to `.audioui` or `.audioui-*`; no unqualified element selectors. Components that render library UI apply `CLASSNAMES.root` on their root element.
...
Audio/MIDI runtime is performance-critical. The Performance Mandate (see Quick Rules) sets the bar; the microbenchmark harness keeps it measurable.

- **Local**: `pnpm bench` runs every bench in the monorepo with tinybench wall-clock measurements. Use for ad-hoc before/after comparisons on a single machine when iterating on a hot path.
- **CI**: `.github/workflows/codspeed.yml` runs `pnpm bench` under `CodSpeedHQ/action@v4` with `mode: instrumentation` (Valgrind/cachegrind). Triggered on push to `main`/`develop` and via `workflow_dispatch`. OIDC authentication — no `CODSPEED_TOKEN` secret is required.
- **Dashboard**: CodSpeed publishes results per-commit on the project dashboard. Use **Compare Any Runs** to diff arbitrary commit pairs (no PR required). Flamegraphs attribute deltas to specific functions, supporting both regression triage and optimization work. Dashboard URL: `https://codspeed.io/cutoff/audio-ui` _(active once the CodSpeed GitHub App is installed; update this line with the canonical URL once known)_.
...
- **Layout**: bench files live in `packages/<pkg>/bench/` (outside `src/` so they stay out of `dist/`). The `*.bench.ts` extension distinguishes them from `*.test.ts`. Vitest discovers them via `benchmark.include` configured in each package's `vitest.config.ts`.
- **Adding a bench**: place a `*.bench.ts` next to the relevant subsystem under `bench/`, mirroring the source layout. Pure framework-agnostic targets are easier to bench cleanly — when the cost lives inside a React component, extract a pure helper (as done with `computeAdaptiveBoxLayout`).
- **Regression policy**: informational only at this stage — no hard merge gates. Reviewers (human and agent) consult the dashboard during the Review procedure (`agents/coding-agent-commands-1.0.md`) and flag instruction-count regressions > 2%. Threshold-based gating graduates later, once a few weeks of data exist.
