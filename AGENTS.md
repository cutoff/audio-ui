...
- **Dashboard**: `https://codspeed.io/cutoff/audio-ui`. Results are published per-commit. Use **Compare Any Runs** to diff arbitrary commit pairs (no PR required). Per-benchmark detail pages chart instruction count over commits on a given branch (require ≥2 valid runs on that branch to render). Flamegraphs attribute deltas to specific functions, supporting both regression triage and optimization work.
