web-monorepo/agents/react-conventions-1.0.md
# React Development Conventions

**Version**: 2.0 | **Last Updated**: Use `now` tool for current timestamp.  
**Meta**: Agent-optimized conventions for React consistency, performance, and maintainability. Focus on functional components/hooks; prefer shared `packages/ui/`. For tasks, grep for "Quick Rules" or "RULE-REACT-001"; apply in TSX edits with `pnpm format`.
...
**Agent Note**: For React edits, enforce functional patterns and shared UI; validate with `pnpm lint` and local tests (light/dark modes).