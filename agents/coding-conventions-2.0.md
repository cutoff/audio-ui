...
**Meta**: Agent-optimized guidelines for consistent code style. Focus on Prettier-enforced formatting; apply via `pnpm format`. For tasks, grep for "Quick Rules" or rules like RULE-FMT-001.
...
> **Note**: Prettier + Tailwind plugin (`prettier-plugin-tailwindcss`) enforces most via `pnpm format`. See `.prettierrc.json` for project settings.
