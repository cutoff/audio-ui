...
| Build               | Vite 6 with declarations (`vite-plugin-dts` 4); outputs `dist/index.js`, `index.d.ts`, `style.css`; ES modules                                                                                                                                                                                                                                                                     |
...
- `dist/`: Built output (`index.js`, `index.d.ts`, `style.css`). `vite.config.ts` sets `rollupOptions.output.assetFileNames` so the bundled stylesheet is always named `style.css`, matching the `package.json` `./style.css` export (Vite 6 library mode otherwise names that chunk `index.css`).
