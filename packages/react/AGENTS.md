...
- `test/`: Test-only code that must never land in `dist/`. Kept out of `src/` so `vite-plugin-dts` does not emit `.d.ts` files for fixtures.
  - `render-count/`: Render-count regression suite (issue #35) — `helpers/renderCounter.tsx` (trackRenders HOC, assertRenderCounts, STRICT flag), `scenes/*.tsx` (MixerGrid, ChannelStrip, StepSequencerRow fixtures), `*.test.tsx`. Typechecked via `tsconfig.test.json`; linted via `eslint src test`. Set `RENDER_COUNT_STRICT=1` to turn baseline divergence into hard failures.
