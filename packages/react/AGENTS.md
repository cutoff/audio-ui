...
| Testing             | Vitest 3; most `.test.tsx` colocated in `src/`; render-count regression scenes + fixtures in `packages/react/test/render-count/` (kept out of `src/` so test code and fixture components never land in `dist/`); React 18/19 compat                                                                                                                                                |
...
- `test/`: Test-only code that must never land in `dist/`. Kept out of `src/` so `vite-plugin-dts` does not emit `.d.ts` files for fixtures.
  - `render-count/`: Memoization regression suite (issue #35). `component-memoization.test.tsx` enforces `React.memo` wrapping on every interactive component, primitive, and view via a static `$$typeof` check. `scenes/*.tsx` fixtures and their `*.test.tsx` files verify scene-level prop stability under value change, parent re-render, theme toggle, and container resize. `helpers/renderCounter.tsx` provides the `trackRenders` HOC, `assertRenderCounts`, and `expectRenderCount` for the scene tests. Typechecked via `tsconfig.test.json`; linted via `eslint src test`.
