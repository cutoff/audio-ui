...
/**
 * Creates an ImageView component with the specified viewBox dimensions.
 ...
 * contract and can be used with BooleanControl.
 *
 * @param {number} frameWidth - Width of the viewBox
 * @param {number} frameHeight - Height of the viewBox
 * @param {number} radius - Radius for the image (typically min(frameWidth, frameHeight) / 2)
 * @param {string} imageHrefFalse - URL to the image for false/off state
 * @param {string} imageHrefTrue - URL to the image for true/on state
 * @returns {ControlComponent<Record<string, never>>} A memoized ControlComponent that renders the image switch
 *
 * @example
 * ```tsx
 * const ViewComponent = createImageView(
 *   100, 100, 45, "/star-off.png", "/star-on.png"
 * );
 * ```
 */
export function createImageView(
    ...
    function DynamicImageView({
        ...
        return (
            <ImageView
                ...
                radius={radius}
                imageHrefFalse={imageHrefFalse}
                imageHrefTrue={imageHrefTrue}
                className={className}
            />
        );
    }

    const MemoizedView = React.memo(DynamicImageView);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (MemoizedView as any).title = "Image Switch";
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (MemoizedView as any).description = "A boolean control that displays one of two images based on value";
