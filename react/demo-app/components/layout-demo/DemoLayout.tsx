...
export default function DemoLayout({
  ...
  children: (props: DemoLayoutRenderProps) => React.ReactNode;
}) {
  // Shared state across demo pages
  const [width, setWidth] = useState(200);
  const [height, setHeight] = useState(300);
  const [shape, setShape] = useState<ShapeValue>("square");
  const [labelPosition, setLabelPosition] = useState<LabelPositionValue>(
    "below"
  );
  const [labelMode, setLabelMode] = useState<LabelModeValue>("visible");
  const [displayMode, setDisplayMode] = useState<DisplayModeValue>(
    "scaleToFit"
  );
  ...
  const [labelHeightUnits, setLabelHeightUnits] = useState<LabelHeightUnitsValue>(
    15
  );
